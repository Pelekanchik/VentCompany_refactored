...
# 🆕 Імпорти для формульного ціноутворення
try:
    from ventilation_company.formula_editor_dialog import FormulaEditorDialog
    from ventilation_company.formula_engine import calculate_price, validate_formula  # noqa: F401
...
class AddProductDialog(tk.Toplevel):
    def __init__(self, parent, product_to_clone: Product | None = None):
        ...
        # 🆕 Поле формули (тільки якщо доступний модуль формул)
        self.formula_var = tk.StringVar(value=clone.formula if clone and clone.formula else "")
        self.auto_price_var = tk.BooleanVar(value=clone.auto_price if clone else False)
        ...
        # 🆕 Режим ціноутворення та формула
        if FORMULA_ENGINE_AVAILABLE:
            frm_mode = tk.LabelFrame(
                self, text="🔧 Режим ціноутворення", font=("Segoe UI", 9, "bold"), padx=10, pady=5
            )
            ...
            tk.Radiobutton(
                ...
                value=False,
                font=("Segoe UI", 9),
                command=self._toggle_formula_field,
            ).pack(anchor="w")
            tk.Radiobutton(
                frm_mode,
                text="🔧 Авто-форма (розрахунок за формулою)",
                variable=self.auto_price_var,
                value=True,
                font=("Segoe UI", 9),
                command=self._toggle_formula_field,
            ...
            tk.Label(frm_mode, text="Формула:", font=("Segoe UI", 9)).pack(anchor="w", pady=(5, 0))
            self.formula_entry = tk.Entry(
                frm_mode, textvariable=self.formula_var, font=("Consolas", 9), width=45
            )
            self.formula_entry.pack(fill="x")
            tk.Label(
                frm_mode,
                text="Приклад: surface_m2 * metal_price * 1.15",
                font=("Segoe UI", 8),
                fg="#666",
            ).pack(anchor="w")

            self._toggle_formula_field()
    ...
    def _toggle_formula_field(self):
        """Увімкнути/вимкнути поле формули."""
    ...
    def _save(self):
        ...
        # 🆕 Отримуємо формулу та режим
        formula = ""
        auto_price = False
        if FORMULA_ENGINE_AVAILABLE and hasattr(self, "formula_var"):
            formula = self.formula_var.get().strip()
            auto_price = self.auto_price_var.get()
...
class ImportDialog(tk.Toplevel):
    ...
    def _import_csv(self):
        ...
        try:
            ...
            msg = "Не вдалося імпортувати CSV:\n" + str(e)
    ...
    def _import_excel(self):
        ...
        try:
            ...
            msg = "Не вдалося імпортувати Excel:\n" + str(e)
            messagebox.showerror("Помилка", msg)
            return
        self.result = products
        self.destroy()
...
class PriceListTab:
    def __init__(self, parent_frame: tk.Frame, root: tk.Tk, colors: dict | None = None):
        ...
        self.col_map = [
            ...
            ("name", "str", True, False, 160),
            ("category", "str", True, True, 100),
            ("length", "float", True, False, 65),
            ("width", "float", True, False, 65),
            ("height", "float", True, False, 65),
            ("diameter", "float", True, False, 70),
            ("material", "str", True, True, 90),
            ("thickness", "float", True, False, 70),
            ("quantity", "float", True, False, 50),
            ("price_per_unit", "float", True, False, 90),
            ("total_price", "float", False, False, 90),
            ("formula", "str", False, False, 120),  # 🆕 Колонка формули
        ]
        ...
        if FORMULA_ENGINE_AVAILABLE:
            tk.Button(
                toolbar, text="🔧 Формула", command=self._open_formula_editor, **btn_cfg
        ...
        if FORMULA_ENGINE_AVAILABLE:
            self.context_menu.add_command(
                label="🔧 Формула ціноутворення", command=self._open_formula_editor
            )
    ...
    def _refresh_table(self):
        ...
        for p in self._filtered_products():
            formula_display = "🔧 авто" if p.auto_price else "✋ ручна"
            if p.formula:
                formula_display = (
                    f"{formula_display}: {p.formula[:20]}..."
                    if len(p.formula) > 20
                    else f"{formula_display}: {p.formula}"
                )
            self.tree.insert(
                "",
                "end",
    ...
    def _save_inline(self, event=None):
        ...
        if self._edit_attr == "price_per_unit" and new_val != product.price_per_unit:
            ...
            # Якщо змінили ціну вручну — вимикаємо авто-формулу
        ...
        # 🆕 Якщо змінили розміри/матеріал/товщину і включена авто-форма — перерахувати
        if (
            product.auto_price
            and product.formula
            and self._edit_attr
            in ("length", "width", "height", "diameter", "thickness", "material", "quantity")
        ):
            try:

                metal_price = self._get_metal_price_per_m2(product.material, product.thickness)
                product.metal_price_per_m2 = metal_price
                product.recalculate_price(metal_price)
                self.status.config(
                    text=f" ✅ Авто-перерахунок: {product.name} = {product.price_per_unit:.2f} грн"
                )
            except Exception as e:
                self.status.config(text=f" ⚠️ Помилка авто-перерахунку: {str(e)}")

        self._dirty = True
        self._refresh_table()
        self.pl.save()
        if "Авто-перерахунок" not in self.status.cget("text"):
    ...
    def _get_metal_price_per_m2(self, material: str, thickness: float) -> float:
        """Повертає ціну металу за м² з налаштувань (грн/м²)."""
        # Типові ціни за м² (грн/м²)
        default_prices = {
            ("цинк", 0.5): 196.25,
            ("цинк", 0.7): 274.75,
            ("цинк", 0.9): 353.25,
            ...
            ("цинк", 1.5): 588.75,
            ("цинк", 2.0): 785.00,
            ("нержавійка", 0.5): 197.50,
            ("нержавійка", 0.7): 276.50,
            ("нержавійка", 0.9): 355.50,
            ("нержавійка", 1.0): 395.00,
            ("нержавійка", 1.2): 474.00,
            ("нержавійка", 1.5): 592.50,
            ("нержавійка", 2.0): 790.00,
            ("алюміній", 0.5): 67.50,
            ("алюміній", 0.7): 94.50,
            ("алюміній", 0.9): 121.50,
            ("алюміній", 1.0): 135.00,
        ...
        mat_key = material.lower() if material else "цинк"
        thick_key = round(float(thickness), 1) if thickness else 0.7

        # Спробуємо завантажити з файлу налаштувань
        try:
            if os.path.exists(PRICING_SETTINGS_FILE):
                with open(PRICING_SETTINGS_FILE, encoding="utf-8") as f:
                    settings = json.load(f)
                prices = settings.get("material_prices_per_m2", {})
                mat_prices = prices.get(mat_key, {})
                thick_str = f"{thick_key} мм"
                if thick_str in mat_prices:
                    return float(mat_prices[thick_str])
        except Exception:
            pass

        # Повертаємо типову ціну
        return default_prices.get((mat_key, thick_key), 274.75)

    def _open_formula_editor(self):
        """Відкриває редактор формули для вибраного виробу."""
        if not FORMULA_ENGINE_AVAILABLE:
            messagebox.showwarning("Увага", "Модуль формул недоступний.")
            return

        ids = self._selected_ids()
        if not ids:
            messagebox.showwarning("Увага", "Виберіть виріб для редагування формули!")
            return
        if len(ids) > 1:
        ...
        def on_save(formula, auto_price):
            product.formula = formula
            product.auto_price = auto_price
            product.metal_price_per_m2 = metal_price
            if auto_price and formula:
                try:
                    product.recalculate_price(metal_price)
                except Exception as e:
                    msg = "Не вдалося розрахувати ціну:\n" + str(e)
                    messagebox.showerror("Помилка", msg)
                    return
            self._dirty = True
            self._refresh_table()
            self.pl.save()
            mode = "авто" if auto_price else "ручна"
            self.status.config(text=f" ✅ Формулу оновлено ({mode}): {product.name}")

        FormulaEditorDialog(self.root, product, metal_price, on_save)
