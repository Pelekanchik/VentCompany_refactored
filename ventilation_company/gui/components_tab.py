...
class ComponentsTab(BaseTab):
    def setup_ui(self):
        ...
        tk.Label(
            proj_bar, text="📁 Проект:", bg=c["bg"], fg=c["fg"], font=("Arial", 12, "bold")
        ).pack(side=tk.LEFT)
        ...
        self.comp_project_combo = ttk.Combobox(
            proj_bar, values=proj_values, width=45, state="readonly", font=("Arial", 11)
        )
        ...
        tk.Button(
            proj_bar,
            text="📤 Передати на розкрій",
            bg="#e67e22",
            fg="white",
            font=("Arial", 10, "bold"),
            cursor="hand2",
            command=self.sync_to_camduct,
        ...
        tk.Button(
            ...
            cursor="hand2",
            command=lambda: (
                self.detail_calculator.calculate_all()
                if hasattr(self, "detail_calculator")
                else None
            ),
        ...
        tk.Button(
            ...
            cursor="hand2",
            command=lambda: (
                self.detail_calculator.export_current_xlsx()
                if hasattr(self, "detail_calculator")
                else None
            ),
        ).pack(side=tk.LEFT, padx=5)
        tk.Button(
            ...
            cursor="hand2",
            command=lambda: (
                self.detail_calculator.print_current()
                if hasattr(self, "detail_calculator")
                else None
            ),
        ).pack(side=tk.LEFT, padx=5)
    ...
    def sync_to_camduct(self):
        ...
        if (
            not camduct_tab
            or not hasattr(camduct_tab, "camduct_editor")
            or camduct_tab.camduct_editor is None
        ):
            messagebox.showwarning(
                "Увага", "Спочатку відкрийте вкладку 'Розкрій листа' хоча б раз."
            )
            return
        count = camduct_tab.camduct_editor.import_calc_items(
            self.detail_calculator.calc_items, clear_existing=True
        )
        if count > 0:
            messagebox.showinfo(
                "Готово",
