...
class MaterialRepo:
    ...
    @staticmethod
    def get_material_by_id(material_id: int) -> sqlite3.Row | None:
        db = get_calc_db()
    ...
    def link_material_to_subtype(
        subtype_id: int, material_id: int, is_default: bool = False
    ) -> None:
        db = get_calc_db()
        db.execute(
    ...
    def set_default_material(subtype_id: int, material_id: int) -> None:
        db = get_calc_db()
        db.execute("UPDATE subtype_materials SET is_default=0 WHERE subtype_id=?", (subtype_id,))
        db.execute(
