...
class ProductRepo:
    ...
    @staticmethod
    def get_type_by_id(type_id: int) -> Optional[sqlite3.Row]:
        db = get_calc_db()
    ...
    def update_type(type_id: int, name: str, slug: str) -> None:
        db = get_calc_db()
        db.execute(
            "UPDATE product_types SET name=?, slug=? WHERE id=?",
            (name, slug, type_id)
        )
        db.commit()
        db.close()
    ...
    def get_subtypes_by_type(type_id: int) -> List[sqlite3.Row]:
        db = get_calc_db()
        rows = db.execute(
            "SELECT * FROM product_subtypes WHERE product_type_id=? ORDER BY name",
            (type_id,)
        ).fetchall()
        db.close()
        return rows
    ...
    @staticmethod
    def get_subtype_by_id(subtype_id: int) -> Optional[sqlite3.Row]:
        db = get_calc_db()
        row = db.execute(
            "SELECT * FROM product_subtypes WHERE id=?", (subtype_id,)
        ).fetchone()
        db.close()
        return row

    @staticmethod
    def add_subtype(type_id: int, name: str, slug: str, formula: str,
                    has_flange: bool = False, labor_norm: float = 0.0,
                    waste_factor: float = 1.0, shape: str = "rect",
                    description: str = "") -> int:
        db = get_calc_db()
        cursor = db.execute(
            """INSERT INTO product_subtypes
               ...
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (type_id, name, slug, formula, int(has_flange),
             labor_norm, waste_factor, shape, description)
        )
        db.commit()
        last_id = cursor.lastrowid
    ...
    def update_subtype(subtype_id: int, **kwargs) -> None:
        db = get_calc_db()
        allowed = {"name", "slug", "formula", "has_flange", "labor_norm",
                   "waste_factor", "shape", "description"}
    ...
    def get_sizes_by_subtype(subtype_id: int) -> List[sqlite3.Row]:
        db = get_calc_db()
        rows = db.execute(
            "SELECT * FROM size_ranges WHERE subtype_id=? ORDER BY param_name",
            (subtype_id,)
        ).fetchall()
        db.close()
        return rows
    ...
    @staticmethod
    def add_size(subtype_id: int, param_name: str, param_label: str,
                 min_value: float = None, max_value: float = None,
                 step: float = None, default_value: float = None) -> int:
        db = get_calc_db()
        cursor = db.execute(
    ...
    @staticmethod
    def update_type_full(type_id: int, name: str, slug: str, sort_order: int = 0, icon: str = "") -> None:
        db = get_calc_db()
        db.execute(
    ...
    @staticmethod
    def get_subtype_full(subtype_id: int) -> Optional[sqlite3.Row]:
        db = get_calc_db()
        row = db.execute(
    ...
    @staticmethod
    def update_subtype_full(subtype_id: int, type_id: int, name: str, slug: str,
                            formula: str, has_flange: bool = False,
                            labor_norm: float = 0.0, waste_factor: float = 1.0,
                            shape: str = "rect", description: str = "") -> None:
        db = get_calc_db()
        db.execute(
            """UPDATE product_subtypes SET
                ...
                WHERE id=?""",
            (type_id, name, slug, formula, int(has_flange),
             labor_norm, waste_factor, shape, description, subtype_id)
        )
        db.commit()
        db.close()
