...
class CalcRepo:
    ...
    def get_all_calculations() -> list[sqlite3.Row]:
        db = get_calc_db()
        rows = db.execute("SELECT * FROM calc_calculations ORDER BY created_at DESC").fetchall()
        db.close()
        return rows

    @staticmethod
    def get_calculation_by_id(calc_id: int) -> sqlite3.Row | None:
        db = get_calc_db()
        row = db.execute("SELECT * FROM calc_calculations WHERE id=?", (calc_id,)).fetchone()
        db.close()
        return row
    ...
    def get_items_by_calculation(calc_id: int) -> list[sqlite3.Row]:
        db = get_calc_db()
        rows = db.execute("SELECT * FROM calc_items WHERE calculation_id=?", (calc_id,)).fetchall()
        db.close()
        return rows
    ...
    def update_calculation_full(
        calc_id: int, total_cost: float = None, sale_price: float = None, status: str = None
    ) -> None:
        db = get_calc_db()
        fields = []
        values = []
        ...
        if fields:
            ...
            try:
                ...
                db.execute(
                    f"UPDATE calc_calculations SET {', '.join(fields_new)} WHERE id=?", values_new
                )
            except sqlite3.OperationalError:
    ...
    @staticmethod
    def get_calculations_by_date_range(start_date: str, end_date: str):
        db = get_calc_db()
        rows = db.execute(
